...
<Tabs>
    <Tab title="Transient errors" icon="rotate">
        Add a retry policy to automatically retry network issues and rate limits. Combine with `timeout=` to cap each attempt. See [Timeout and error handling](/oss/python/langgraph/timeout-and-error-handling) for the full lifecycle.
    ...
    <Tab title="Saga / compensation" icon="arrows-exchange">
        After retries are exhausted, run a recovery function that updates state and routes to a compensation branch. See [Timeout and error handling](/oss/python/langgraph/timeout-and-error-handling#add-an-error-handler) for the full pattern.
