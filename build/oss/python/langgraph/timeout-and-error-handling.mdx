...
sidebarTitle: Timeout and error handling
...
These compose in a fixed order: a timeout fires first (raising [`NodeTimeoutError`](https://reference.langchain.com/python/langgraph/errors/NodeTimeoutError)), then the retry policy decides whether to retry, and only after retries are exhausted does the error handler fire.
...
flowchart LR
    ...
    exec -->|"raises"| retry{retry_policy<br/>matches?}
    exec -->|"timeout fires"| timeoutErr[NodeTimeoutError]
    timeoutErr --> retry
    ...
    class exec,run_handler,timeoutErr process
...
See [Add retry policies](/oss/python/langgraph/use-graph-api#add-retry-policies) for the full retry API, including `retry_on`, `initial_interval`, and `backoff_factor`.

## Configure a node timeout

The `timeout=` parameter on [`add_node`](https://reference.langchain.com/python/langgraph/graph/state/StateGraph/add_node) caps how long a single node attempt may run. When the limit is exceeded, LangGraph raises [`NodeTimeoutError`](https://reference.langchain.com/python/langgraph/errors/NodeTimeoutError), clears any writes from the failed attempt, and lets the retry policy decide whether to retry.

Pass a number (seconds), a `timedelta`, or a [`TimeoutPolicy`](https://reference.langchain.com/python/langgraph/types/TimeoutPolicy) for finer control:
...
[`TimeoutPolicy`](https://reference.langchain.com/python/langgraph/types/TimeoutPolicy) supports two complementary limits:
...
`NodeTimeoutError` is a subclass of `TimeoutError`. If you want timed-out attempts to be retried, include `TimeoutError` (or `NodeTimeoutError`) in your retry policy's `retry_on`:
...
[`NodeTimeoutError`](https://reference.langchain.com/python/langgraph/errors/NodeTimeoutError) carries structured context about the timeout that fired:
...
An error handler is a function that runs after a node fails and all retries are exhausted. It receives the current state and can update it or route to a different node using [`Command`](https://reference.langchain.com/python/langgraph/types/Command). This is useful for compensation flows (e.g., Saga patterns) where you want to recover gracefully rather than abort the entire graph.
...
<Note>
Per-node timeouts require `langgraph>=<TBD>`.
</Note>

<Note>
Node-level error handlers require `langgraph>=<TBD>`.
</Note>
...
<Callout icon="edit">
    [Edit this page on GitHub](https://github.com/langchain-ai/docs/edit/main/src/oss/langgraph/timeout-and-error-handling.mdx) or [file an issue](https://github.com/langchain-ai/docs/issues/new/choose).
</Callout>
</div>
