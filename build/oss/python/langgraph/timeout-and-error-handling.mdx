...
These compose in a fixed order: when a node attempt fails with any exception (including [`NodeTimeoutError`](https://reference.langchain.com/python/langgraph/errors/NodeTimeoutError) from a timeout), the retry policy decides whether to retry, and only after retries are exhausted does the error handler fire.
...
See [Add retry policies](/oss/python/langgraph/use-graph-api#add-retry-policies) for the full retry API, including `initial_interval` and `backoff_factor`.
