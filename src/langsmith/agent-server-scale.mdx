...
<Warning>
Enabling `BG_JOB_ISOLATED_LOOPS` prevents health-check failures caused by a blocked serving loop, but the synchronous code continues to run on the background loop and will still degrade throughput, tail latency, and scaling under load. To actually resolve those issues, use native async drivers and async code throughout your agent, or wrap unavoidable sync calls in `asyncio.to_thread(...)`. See [`BG_JOB_ISOLATED_LOOPS`](/langsmith/env-var#bg_job_isolated_loops) for details.
</Warning>
