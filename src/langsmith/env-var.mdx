...
<Warning>
Enabling this flag does not remove the underlying problem. It moves synchronous blocking work off the serving API's event loop so health checks stop failing, but the blocking code continues to run on the background loop and continues to cause production issues: degraded throughput, tail-latency spikes, starved workers, connection pool exhaustion (see the pool-size caveat below), and poor scaling under load.

To actually resolve those issues, use native async drivers and async code throughout your agent. That means async HTTP clients like `httpx` or `aiohttp`, async database drivers like `asyncpg` or `psycopg[async]`, async model SDKs, and `await asyncio.sleep(...)` in place of `time.sleep(...)`. For unavoidable synchronous libraries, wrap the specific call in `asyncio.to_thread(...)` or `loop.run_in_executor(...)` instead of enabling this flag for the whole deployment.
</Warning>
