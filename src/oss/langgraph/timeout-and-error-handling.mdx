...
These compose in a fixed order: a timeout fires first (raising @[`NodeTimeoutError`]), then the retry policy decides whether to retry, and only after retries are exhausted does the error handler fire.
...
See [Add retry policies](/oss/langgraph/use-graph-api#add-retry-policies) for the full retry API, including `retry_on`, `initial_interval`, and `backoff_factor`.
