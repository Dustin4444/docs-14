...
See [Timeout and error handling](/oss/langgraph/timeout-and-error-handling) for the full timeout lifecycle, idle-timeout refresh sources, and `runtime.heartbeat()`.
...
See [Timeout and error handling](/oss/langgraph/timeout-and-error-handling#add-an-error-handler) for compensation patterns and `Command` routing.
